...
export class TimetableService {
    ...
    async generateTimetable(username: string, desiredSubjects: Array<string>) {
        ...
        const population = this.createTimetablePopulation(
            classList,
            classIDGroup,
        );
        const { timetable_population, priority_point } = this.createTimetable(
            classList,
            population,
        );
        const low_res = this.getLowDensityTimetable(
            priority_point,
            timetable_population,
        );
        const high_res = this.getHighDensityTimetable(
            priority_point,
            timetable_population,
        );
    ...
    private async getClassList(
        username: string,
        desiredSubjects: Array<string>,
    ): Promise<Array<Class>> {
        const subjects: Array<any> = await this.cacheManager.get(
            `csv_${username}`,
        );
        const result = [];
    ...
    private getClassIDGroup(classes: Array<Class>) {
        const classIDArr = [];
        const unique_module_id = Array.from(
            new Set(classes.map((item) => item.moduleID)),
        );
    ...
    private createTimetablePopulation(
        classes: Array<Class>,
        class_id_group: Array<string>,
    ) {
        const cartesian = (...a) =>
        ...
        for (const element of timetable) {
            if (element.length > 1) {
                for (let n = 0; n < element.length - 1; n++) {
                    ...
                    empty_time =
                        empty_time +
                        Math.abs(Number(start_time) - Number(end_time));
                }
            }
            if (element.length > 0) {
    ...
    private getLowDensityTimetable(
        priority_point: Array<number>,
        timetable: Array<Array<any>>,
    ) {
        let i;
    ...
    private getHighDensityTimetable(
        priority_point: Array<number>,
        timetable: Array<Array<any>>,
    ) {
        let i;
